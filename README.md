...
### SEO / GEO — structured data, кейсы, метаданные, llms.txt (22 августа 2026)

Полный проход по SEO/GEO-чек-листу: технический аудит существующей разметки, кейс-тексты по каждому проекту, structured data, метаданные под реальные лимиты SERP и файл для AI-агентов.

**Structured data**

- На `/projects` добавлена схема `CreativeWork` (обёрнутая в `ItemList`) по каждому проекту — `name`, `description`, `url`, `keywords`, `dateCreated`, `author`, собранная из уже существующих данных `projects.js`, без ручного дублирования контента. Дополняет схемы `Person`/`WebSite` (статика в `index.html`) и `BreadcrumbList` (About/Projects).
- В `<h1>` hero-секции добавлен скрытый (`visually-hidden`) keyword-спан с полным `metadata.home.title` — краулеры и AI-выдача видят ключевые слова из тайтла прямо в заголовке, видимый тэглайн-афоризм не тронут.

**Кейс-тексты вместо маркетинговых однострочников** — у всех 8 проектов `overview` переписан по схеме задача → стек → результат на реальных фактах: mobile-first вёрстка 65 страниц и запуск vam-voda.com, нативная (без React/Next) разработка waterdel.store в сжатые сроки, продакшен-стажировка в диджитал-студии PurpleSchool на Ratingus (фича + API + отдельная страница, Scrum), первый фриланс-заказ на CreateX. Даёт поисковикам и AI-выдаче уникальный индексируемый текст вместо шаблонных фраз.

**Метаданные**

- `public/og-cover.png` (1200×630, фирменный градиент из hero) — раньше `og:image`/`twitter:image` указывали на несуществующий файл, превью при шеринге ссылки было битым.
- `title`/`description` для Home и About обрезаны под практические лимиты SERP (~60 / ~155 символов) — были 178–194 символа и рисковали обрезаться Google на "...".

**`public/llms.txt`** — markdown-файл по конвенции [llms.txt](https://llmstxt.org): заголовок, краткое описание сайта и ссылки на страницы специально для AI-агентов/LLM-краулеров (не для людей и не для классических поисковых роботов). Закрывает чек Lighthouse **Agentic Browsing**.

**Иконки PWA** — `logo192.png`/`logo512.png` заменены с дефолтного логотипа-атома Create React App (никак не связанного с сайтом) на брендированную иконку: скруглённый квадрат в градиенте hero, буква «A», акцент-каретка как отсылка к мигающему курсору в текстовом логотипе.

#### Метрики Lighthouse (desktop, прод-сборка)
...
> [!NOTE]
> Замер — desktop-профиль **без искусственного троттлинга** сети/CPU, на прод-сборке (`npm run prod`). Под симулированным mobile-throttling (условия среднего телефона на 4G — профиль по умолчанию у Lighthouse CLI) Performance проседает до ~75–94/100 в зависимости от точного профиля: desktop-цифры показывают потолок на быстром соединении, а не гарантию на всех устройствах и сетях.

---

### Ревизия адаптивности и вёрстки (21 августа 2026)

Точечный проход по адаптивности главной страницы — от планшета до мобильных — плюс редизайн секции услуг.

**Витрина проектов (`StickyShowcase`)**

- `isPinned` (класс, скрывающий хедер при закреплённой секции) вынесен из побочного эффекта внутри обработчика скролла в отдельный React state + `useEffect`
- Непрерывный индекс прокрутки для кроссфейда карточек убран из React state — пишется императивно в CSS custom property на каждый кадр, сам кроссфейд считает `calc()`/`clamp()` в CSS вместо ре-рендера по 60 раз в секунду
- Устранён «прыжок» картинки при смене активного проекта (переменная высота слогана резервировалась `min-height`)
- На планшете (768–1024px) картинка проекта заполняет доступную высоту без обрезки контента — блок под картинку получил `aspect-ratio`, совпадающий с самим изображением, вместо `object-fit: cover`, который на портретных пропорциях обрезал текст/лого прямо на скриншотах продуктов
- Точки-индикатор получили тёмную полупрозрачную подложку с блюром — на светлых/пёстрых скриншотах белые точки без подложки были почти не видны

**Hero** — секция снова занимает весь экран (`100dvh`) на всех размерах, включая мобильные; ранее на ≤768px высота подстраивалась под контент и открывала часть следующей секции на одном экране.

**Отступ под хедером** (`/about`, `/projects`) — верхний паддинг страниц привязан к `calc(var(--header-height) + 1rem)` вместо плоских `rem`-значений: хлебные крошки раньше могли оказаться ýже, чем сам фиксированный хедер, теперь отступ гарантированно больше его высоты на любом брейкпоинте.

**«Почему именно я»** — сетка преимуществ держит 2 колонки до 568px вместо 768px: на планшете короткие подписи занимали почти всю ширину карточки вместо разумных ~350px.

**«Мои услуги» — редизайн.** Сетка из одинаковых карточек «иконка + заголовок + текст» заменена на типографский список: каждая услуга — строка на всю ширину с hairline-разделителем, имя услуги на масштабе заголовка секции, подсказка «Обсудить проект» на hover скроллит к форме контактов. Заодно добавлены две услуги: AI-ускоренная разработка и Backend/данные (растущий стек — Python и базы данных).

**Форма контактов** — подсказка «\* — обязательное поле» перенесена из левого верхнего угла формы вниз направо, к кнопке отправки.
...
> [!NOTE]
> Первые два пункта ниже — исторические, замерены на состоянии июня 2026. Оба поведения изменены в августе 2026 (см. запись ниже): Hero снова `100dvh` на всех размерах, а `.projects`/`.about` считают верхний отступ от реальной высоты хедера, а не от плоского `4rem`/`3rem`.
...
- 🔍 **SEO / GEO** — нативные метаданные React 19 (`title`, `description`, `canonical`, `hreflang`, Open Graph), локализованные мета-теги, JSON-LD `Person`/`WebSite`/`BreadcrumbList`/`CreativeWork` по каждому проекту, `public/llms.txt` для AI-агентов.
