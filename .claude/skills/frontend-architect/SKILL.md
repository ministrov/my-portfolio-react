...
- **Mobile-first** — базовые стили для мобильного, расширение через `min-width`.
...
Брейкпоинт ставится там, где **ломается макет**, а не под конкретный девайс. Сначала
выжать максимум из `clamp`/`auto-fit`/container queries — медиа-запрос только когда меняется
сама раскладка (1 колонка → 2). Хранить общие точки согласованно (как `max-width: 375px`
в `Up`).
...
- [ ] Mobile-first (`min-width`), логические свойства, `gap`?
